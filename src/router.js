...
//Requisições GET
routes.get("/", new HomepageServerController().handle); //Homepage
routes.get("/api/users", databaseUsers, new UsersListController().handle); //Lista de usuários
routes.get("/api/users/:_id/logs", databaseUsers, new LogsListController().handler); //Logs de um usuário especifico
...
routes.post("/api/users/:_id/exercises", databaseUsers, new NewExerciseController().handle); //cria um novo exercício de um novo usuário
